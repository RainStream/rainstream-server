...
PipeStreamSocket::PipeStreamSocket(size_t bufferSize) : bufferSize(bufferSize)
{
	MS_TRACE_STD();
	...
	if (err != 0)
	{
		...
		RS_THROW_ERROR_STD("uv_pipe_init() failed: %s", uv_strerror(err));
	}

	// 	err = uv_pipe_open(this->uvHandle, 0);
	// 	if (err != 0)
	// 	{
	// 		//uv_close(reinterpret_cast<uv_handle_t*>(this->uvHandle), static_cast<uv_close_cb>(onErrorClose));
	// 
	// 		RS_THROW_ERROR_STD("uv_pipe_open() failed: %s", uv_strerror(err));
	// 	}
	//  
	//    	// Start reading.
	//   	err = uv_read_start(
	...
	//   		uv_close(reinterpret_cast<uv_handle_t*>(this->uvHandle), static_cast<uv_close_cb>(onErrorClose));
	//   
	//   		RS_THROW_ERROR_STD("uv_read_start() failed: %s", uv_strerror(err));
	//   	}
